...
const PublicEventPage: React.FC<PublicEventPageProps> = ({
  params,
  navigate,
}) => {
  ...
  useEffect(() => {
    if (event) {
      ...
      updateMetaTag(
        "keywords",
        `event, ${event.team.name}, ${event.eventType}, ${event.venue}`
      );
      ...
      if (event.socialImageUrl || event.bannerImageUrl) {
        updateMetaProperty(
          "og:image",
          event.socialImageUrl || event.bannerImageUrl
        );
      }
  ...
  const isAtCapacity =
    event.maxCapacity && registrationCount >= event.maxCapacity;
  const registrationDeadlinePassed =
    event.registrationDeadline && event.registrationDeadline <= Date.now();
        ...
        <div className="max-w-4xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
              {/* Registration Card */}
              <div
                className="bg-background rounded-lg p-6 shadow-sm border-l-4"
                style={{
                  borderLeftColor: event.team.primaryColor || "#3b82f6",
                }}
              >
                    ...
                    {event.maxCapacity && (
                      <div>
                        <div className="flex justify-between text-sm text-muted-foreground mb-1">
                          ...
                          <span>
                            {event.maxCapacity - registrationCount} left
                          </span>
                        </div>
                        <div className="w-full bg-muted rounded-full h-2">
                          <div
                            ...
                            style={{
                              ...
                              backgroundColor:
                                event.team.primaryColor || "#3b82f6",
                            }}
                          />
                        </div>
                      </div>
...
const EventNotFound: React.FC<{ navigate: (to: string) => void }> = ({
  navigate,
}) => {
  return (
