...
export const validateEventField = (
  ...
  switch (field) {
    ...
    case "description": {
      ...
      if (
        trimmedDescription.length < eventValidationRules.description.minLength
      ) {
        return `Description must be at least ${eventValidationRules.description.minLength} characters`;
      }
      if (
        trimmedDescription.length > eventValidationRules.description.maxLength
      ) {
    ...
    case "startTime": {
      ...
      maxBookingDate.setDate(
        maxBookingDate.getDate() +
          eventValidationRules.dateTime.maxAdvanceBooking
      );
    ...
    case "endTime":
      ...
      if (
        formData?.startTime &&
        value.getTime() <= formData.startTime.getTime()
      ) {
