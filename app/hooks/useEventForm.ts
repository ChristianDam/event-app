...
export const useEventForm = ({
  ...
  useEffect(() => {
    const subscription = watch((value, { name }) => {
      if (name === "startTime" && value.startTime) {
        ...
        const currentEndTime = value.endTime
          ? new Date(value.endTime)
          : undefined;
        const currentStartTime = watchedValues.startTime
        ...
        if (currentStartTime && currentEndTime) {
          ...
          if (
            ...
            currentStartTime.getTime() + 2 * 60 * 60 * 1000
          ) {
            setValue(
              "endTime",
  ...
  const validateField = useCallback(
    async (field: keyof EventFormData): Promise<string | null> => {
